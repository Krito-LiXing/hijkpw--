...
https://m.81zw.net/
https://m.xbiquge.so/
https://m.liehuozw.com/
https://www.haitang123.tv/
https://m.mht99.com/
https://m.ijjxsw.com/
https://m.114zww.com/
https://m.sinodan.cc/
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        [[ -f ~/.acme.sh/${DOMAIN}_ecc/ca.cer ]] || {
            colorEcho $RED " 获取证书失败，请复制上面的红色文字到 https://hijk.art（以关闭） 反馈"
            exit 1
        }
        CERT_FILE="/etc/v2ray/${DOMAIN}.pem"
        KEY_FILE="/etc/v2ray/${DOMAIN}.key"
        ...
        [[ -f $CERT_FILE && -f $KEY_FILE ]] || {
            colorEcho $RED " 获取证书失败，请到 https://hijk.art（以关闭） 反馈"
            exit 1
        }
    else
...
menu() {
    ...
    echo -e "# ${GREEN}作者${PLAIN}: 网络跳越(hijk)（已退网）                            #"
    echo -e "# ${GREEN}网址${PLAIN}: （已关闭）                                          #"
    echo -e "# ${GREEN}论坛${PLAIN}: （已关闭）                                          #"
    echo -e "# ${GREEN}TG群${PLAIN}: （已注销）                                          #"
    echo -e "# ${GREEN}Youtube频道${PLAIN}: （已注销）                                   #"
    echo "#############################################################"
